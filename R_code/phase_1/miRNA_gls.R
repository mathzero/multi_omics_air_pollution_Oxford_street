...
###############################################################
###############################################################
work_dir<-"~/Google Drive/Imperial/2 Translational data science/TDS project/Project6/Results/phase_1/results_miRNA/05_sensitivity"
setwd(work_dir)
...
miRNA.full.sig <- cbind(miRNA.list[[1]][1:2], miRNA.list[[2]][2],miRNA.list[[3]][2], miRNA.list[[4]][2], miRNA.list[[5]][2])

colnames(miRNA.full.sig) <- c("OMICid","NO2_pval","PM10_pval","PM25_pval","PCNT_pval","CBLK_pval")
write.csv(miRNA.full.sig, "miRNA_all_pvals.csv")

##############################################################################################
##############################################################################################

### Extracting list of significant miRNAs and associated TRAP ####


miRNA.list[[1]][miRNA.list[[1]]$pvalBH < 0.05,1:4]
...
write.table(significant.omics, "significant_miRNA_denoised.txt", col.names = T, row.names = FALSE)
write.csv(significant.omics, "significant_miRNA_denoised.csv")

plots.list <- list()

for (i in 1:length(exposures.miRNA)){
  plots.list[[i]] <- ggplot(data=miRNA.list[[i]], aes(x=beta, y=-log10(pval), col = -log10(pvalBH) > -log10(0.05))) +
...
png("miRNA_volcanos.png",width = 8000, height = 2500, res = 300)
